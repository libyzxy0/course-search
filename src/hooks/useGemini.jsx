...
const useGemini = () => {
  ...
  const analyzeSurvey = async (ans) => {
    ...
    const prompt = `
      ...
      Hello, I want you to analyze the user answers on questions and give me at most 5 possible courses. The answers maybe tagalog or English just translate it Make it accurate based on the output on answer { question, answer } key.
      [END ANALYSIS PROMPT]\n\n
    ...
    try {
      ...
      if (parsedResponse) {
        for (let i = 0; i < parsedResponse.length; i++) {
          await createCourse(parsedResponse[i]);
        }
