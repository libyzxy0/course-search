...
const Login = () => {
  ...
  return (
    ...
    <header className="mx-8 pt-16">
      <h1 className="text-gray-700 text-3xl font-bold">Sign Up | <b className="text-emerald-400">Course</b>search</h1>
    </header>
    <div className="h-screen w-full bg-white flex flex-col md:flex-row md:justify-between">
      ...
      <div className="pt-16 md:pt-20 flex flex-col w-full md:w-[50%]">
        ...
        <div className="flex flex-col mx-8">
          ...
          <input onChange={(e) => setFirstName(e.target.value)} value={firstName} className="outline-none py-2 rounded-lg hover:border-emerald-400 mt-2 border-[1.5px] border-gray-200 px-4" type="email" placeholder="John" />
        </div>
        <div className="flex flex-col mx-8 mt-4">
          <label className="font-medium text-gray-700">Last Name</label>
          <input onChange={(e) => setLastName(e.target.value)} value={lastName} className="outline-none py-2 rounded-lg hover:border-emerald-400 mt-2 border-[1.5px] border-gray-200 px-4" type="email" placeholder="Doe" />
        </div>
        <div className="flex flex-col mx-8 mt-4">
        ...
        <div className="flex flex-col mx-8 mt-4 relative">
          ...
          <input onChange={(e) => setConfirmPassword(e.target.value)} value={confirmPassword} className="outline-none py-2 rounded-lg hover:border-emerald-400 mt-2 border-[1.5px] border-gray-200 px-4" type={ showc ? 'text' : 'password'} placeholder="Mypassword@123" />
          {confirmPassword && (
            <button onClick={() => setShowC(!showc)} className="absolute top-[2.6rem] right-[0.8rem] text-gray-700">
            { showc  ? <Eye /> : <EyeOff /> }
            </button>
          )}
        ...
        <div className="flex flex-col mx-8 mt-5">
          <button onClick={handleSignUp} className="w-full border-none rounded-lg bg-emerald-400 py-2.5 text-white font-medium flex justify-center">{
            loading ? (
        ...
        <h1 className="text-center mt-6 text-gray-600">Already have an account? <Link className="text-emerald-400 hover:underline" to="/login">Sign In</Link></h1>
      </div>
